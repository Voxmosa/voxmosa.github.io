...
//
// ⚠️ 四個頁面都已烘焙完畢，這支工具目前沒有對象，保留是為了兩件事：
//    記錄當初怎麼做的，以及萬一日後又出現 <x-dc> 頁面時可以重用。
//
// 用戶端渲染的存在理由是 Claude Design 這個視覺編輯器。編輯器退場後這層成本
// 就換不到東西了，所以把版面固定成靜態 HTML，連帶移除了 support.js（69KB）
// 與 React（142KB），首屏不必等 JS，白畫面風險與兜底看門狗也一併消失。
//
// 做法是「改寫原始碼結構」，而不是「輸出渲染後的 DOM」：
// 渲染結果是一整行沒有斷行的 HTML，之後沒人能維護；原始模板本來就排版良好，
